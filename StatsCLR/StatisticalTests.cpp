...
namespace StatsCLR
{
	...
	TTest::TTest(double mu0, double x_bar, double sx, double n)
	{
		...
		m_test = new StatisticsLibrary::TTest(mu0, x_bar, sx, n);
	...
	TTest::TTest(double mu0, List<double>^ x1)
	{
		...
		m_test = new StatisticsLibrary::TTest(mu0, _x1);
	...
	TTest::TTest(List<double>^ x1, List<double>^ x2)
	{
		...
		m_test = new StatisticsLibrary::TTest(_x1, _x2);
	...
	ZTest::ZTest(double mu0, double x_bar, double sx, double n)
	{
		...
		m_test = new StatisticsLibrary::ZTest(mu0, x_bar, sx, n);
	...
	// One-sample: population mean, sample.
	//
	ZTest::ZTest(double mu0, List<double>^ x1)
	{
		STATS_TRY

		std::vector<double> _x1 = Conversion::ListToVector(x1);
		m_test = new StatisticsLibrary::ZTest(mu0, _x1);
	...
	// Two-sample
	//
	ZTest::ZTest(List<double>^ x1, List<double>^ x2)
	{
		STATS_TRY
		...
		m_test = new StatisticsLibrary::ZTest(_x1, _x2);
	...
	// Retrieve the results
	//
	Dictionary<String^, double>^ ZTest::Results()
	{
		STATS_TRY
		const auto _results = m_test->Results();
		Dictionary<String^, double>^ results = Conversion::ResultsToDictionary(_results);
