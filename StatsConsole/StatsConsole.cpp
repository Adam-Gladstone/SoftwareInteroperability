...
namespace StatsConsole
{
    ...
    //
    StatisticsLibrary::DataManager g_data_manager;
    ...
    bool OnListDataSets(const std::string& prompt)
    {
        ...
        std::vector<StatisticsLibrary::DataSetInfo> data_sets = g_data_manager.ListDataSets();
        if (!data_sets.empty())
        {
    ...
    bool OnDescriptiveStats(const std::string& prompt)
    {
        ...
        if (!name.empty())
        {
            ...
            if (!data.empty())
            {
                const auto results = StatisticsLibrary::GetDescriptiveStatistics(data);
    ...
    bool OnLinearRegression(const std::string& prompt)
    {
        ...
        if (!indep_name.empty() && !dep_name.empty())
        {
            ...
            else
            {
                ...
                const auto results = StatisticsLibrary::LinearRegression(xs, ys);
                OutputResults(results);
            }
        }
        return true;
    ...
    bool OnSummaryDataTtest(const std::string& prompt)
    {
        ...
        StatisticsLibrary::TTest test(mu0, x_bar, sx, n);
    ...
    bool OnSummaryDataFtest(const std::string& prompt)
    {
        ...
        StatisticsLibrary::FTest test(sx1, sx2, n1, n2);
        const bool success = test.Perform();
        if (success)
        {
            const auto results = test.Results();
    ...
    bool OnTwoSampleFtest(const std::string& prompt)
    {
        ...
        if (!x1_name.empty() && !x2_name.empty())
        {
            ...
            {
                StatisticsLibrary::FTest test(x1, x2);
                const bool success = test.Perform();
                if (success)
                {
                    const auto results = test.Results();
    ...
    bool OnMovingAverage(const std::string& prompt)
    {
        Output o(prompt);

        // Get the name of a previously loaded dataset
        const std::string obs = GetName();
        const double window = GetDouble("Window");

        if (!obs.empty())
        {
            std::vector<double> observations = g_data_manager.GetDataSet(obs);
            if (observations.empty())
            {
                std::cout << "There is no data in the data set: " << obs << ".\n";
            }
            else
            {
                // Generate (or otherwise invent) the dates - could read in two columns
                std::vector<StatisticsLibrary::Date> dates(observations.size());
                std::iota(dates.begin(), dates.end(), 1);

                // Create an instance of the TimeSeries class passing in dates and observations
                StatisticsLibrary::TimeSeries ts(dates, observations);

                // Compute the moving average and present the results
                const auto results = ts.MovingAverage(static_cast<int>(window));
                std::size_t i = 0;
                for (const auto& result : results)
                {
                    std::cout << std::setw(15) << "[" << dates[i++] << "]:\t" << result << "\n";
                }
            }
        }
        return true;
    ...
    MenuItems mainMenu =
    {
        ...
        {"12", {"Moving Average",	        OnMovingAverage } },
        {"13", {"Quit",						OnClose } }
    };
}
