...
void CStatsViewerForm::OnStatisticsDescriptive()
{
	...
	else
	{
		...
		try
		{
			std::unordered_map<std::string, double> results = StatisticsLibrary::GetDescriptiveStatistics(data);
...
void CStatsViewerForm::OnStatisticsLinearRegression()
{
	...
	try
	{
		std::unordered_map<std::string, double> results = StatisticsLibrary::LinearRegression(xs, ys);
		PopulateListControl(m_lvResults, results);
...
std::shared_ptr<StatisticsLibrary::StatisticalTest> 
CStatsViewerForm::MakeTest(const StatsViewer::TTestParams& params, const CStringArray& arrDataSetNames, CString& strLabel) const
{
	...
	std::shared_ptr<StatisticsLibrary::StatisticalTest> test = nullptr;
	...
	switch (params.test_type)
	{
		case TTestParams::TestType::SummaryData:
		{
			...
			try
			{
				test = std::make_shared<StatisticsLibrary::TTest>(params.mu0, params.xbar, params.sx, params.n);
			}
			catch (const std::exception& e)
			{
				ReportException(e);
		...
		case TTestParams::TestType::OneSample:
		{
			...
			try
			{
				...
				test = std::make_shared<StatisticsLibrary::TTest>(params.mu0, x1);
			}
			catch (const std::exception& e)
			{
				ReportException(e);
		...
		case TTestParams::TestType::TwoSample:
		{
			...
			try
			{
				...
				test = std::make_shared<StatisticsLibrary::TTest>(x1, x2);
			}
			catch (const std::exception& e)
			{
				ReportException(e);
...
CStatsViewerForm::MakeTest(const StatsViewer::FTestParams& params, const CStringArray& arrDataSetNames, CString& strLabel) const
{
	...
	switch (params.test_type)
	{
		case FTestParams::TestType::SummaryData:
		{
			...
			try
			{
				test = std::make_shared<StatisticsLibrary::FTest>(params.sx1, params.sx2, params.n1, params.n2);
			}
			catch (const std::exception& e)
			{
				ReportException(e);
		...
		case FTestParams::TestType::TwoSample:
		{
			...
			try
			{
				...
				test = std::make_shared<StatisticsLibrary::FTest>(x1, x2);
			}
			catch (const std::exception& e)
			{
				ReportException(e);
...
void CStatsViewerForm::PerformTest(const std::shared_ptr<StatisticsLibrary::StatisticalTest>& test, const CString& strLabel)
{
	if (test == nullptr)
	{
		CString strMessage;
...
void CStatsViewerForm::OnStatisticsTestsTtest()
{
	...
	CString strLabel;
	std::shared_ptr<StatisticsLibrary::StatisticalTest> test = MakeTest(dlg.GetParams(), CStringArray(), strLabel);
...
void CStatsViewerForm::OnStatisticsTestsOneSample()
{
	...
	std::shared_ptr<StatisticsLibrary::StatisticalTest> test = MakeTest(dlgInputs.GetParams(), arrDataSetNames, strLabel);
	PerformTest(test, strLabel);
}
...
void CStatsViewerForm::OnStatisticsTestsTwoSample()
{
	...
	CString strLabel;
	const CStringArray& arrDataSetNames = dlg.GetSelectedDataSetNames();
	std::shared_ptr<StatisticsLibrary::StatisticalTest> test = MakeTest(params, arrDataSetNames, strLabel);
	PerformTest(test, strLabel);
}
